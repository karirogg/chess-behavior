...
def get_first_k_moves(use_username = True, username= None, path = None, k = 5, restrict_games = False, num_games = 10):
    ...
    while True:
        ...
        print(game)
        if game == None or (restrict_games and n > num_games):
            break
        n+=1
...
def get_concat_k_moves(dir, k = 5, restrict_games = False, num_games = 10):
    list_of_df = []

    for game_file in os.listdir(dir):
        path = f'./{dir}/{game_file}'
        new_df = get_first_k_moves(use_username = False, path = path, num_games = num_games, restrict_games = restrict_games)
...
def generate_X_y(dir, debug = False):
    df = get_concat_k_moves(dir, 20, restrict_games = debug, target_date = '2012')
    print(df.shape)
    X,y = df_to_numpy(df)
